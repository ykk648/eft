...
from eft.apps.eval import run_evaluation  # For test
from eft.utils.timer import Timer

g_timer = Timer()

from eft.utils.data_loader import CheckpointDataLoader
from eft.utils.saver import CheckpointSaver

from eft.utils.imutils import convert_smpl_to_bbox, convert_bbox_to_oriIm, conv_bboxinfo_center2topleft, \
    deNormalizeBatchImg
...
from eft.utils.pose_utils import reconstruction_error, reconstruction_error_fromMesh
...
class EFTFitter(Trainer):
    ...
    # Use openpose hip definition
    def keypoint_3d_hand_loss_panopticDB(self, pred_right_hand_joints_3d, pred_left_hand_joints_3d, gt_lhand_3d,
                                         gt_rhand_3d):
        """Compute 3D keypoint loss for the examples that 3D keypoint annotations are available.
        The loss is weighted by the confidence.
        """
    ...
    def run_eft_step(self, input_batch, iterIdx=0):
        ...
        pred_rotmat, pred_betas, pred_camera, pred_feature = self.model(images, return_feature=True)
        ...
        # g_timer.toc("EFT time")
        g_timer.toc(average=False, bPrint=True, title="EFT iter")
    ...
    def run_eft_step_with_2dhand(self, input_batch, iterIdx=0):
        ...
        pred_output = self.smpl(betas=pred_betas, body_pose=pred_rotmat[:, 1:], global_orient=pred_rotmat[:, [0]],
                                pose2rot=False)
        pred_vertices = pred_output.vertices
        pred_joints_3d = pred_output.joints
        ...
        pred_right_hand_2d = weakProjection_gpu(pred_output.right_hand_joints, pred_camera[:, 0],
                                                pred_camera[:, 1:])  # N, 49, 2
        pred_left_hand_2d = weakProjection_gpu(pred_output.left_hand_joints, pred_camera[:, 0],
                                               pred_camera[:, 1:])  # N, 49, 2
        ...
        loss_keypoints_2d_hand = self.keypoint_loss_keypoint21(pred_right_hand_2d, gt_kp_rhand_2d,
                                                               1.0) + self.keypoint_loss_keypoint21(pred_left_hand_2d,
                                                                                                    gt_kp_lhand_2d, 1.0)
        ...
        if self.options.bDebug_visEFT:  # g_debugVisualize:    #Debug Visualize input
            ...
            for b in range(batch_size):
                ...
                if True:
                    ...
                    curImgVis = viewer2D.Vis_Skeleton_2D_Openpose_hand(
                        (gt_kp_rhand_2d[0, :, :2].cpu().numpy() + 1) * 112, gt_kp_rhand_2d[0, :, 2].cpu().numpy(),
                        image=curImgVis)
                    curImgVis = viewer2D.Vis_Skeleton_2D_Openpose_hand(
                        (gt_kp_lhand_2d[0, :, :2].cpu().numpy() + 1) * 112, gt_kp_lhand_2d[0, :, 2].cpu().numpy(),
                        image=curImgVis)
    ...
    def run_eft_step_wHand(self, input_batch):
        ...
        if self.options.bUseHand3D:
            loss_keypoints_3d_hand = self.keypoint_3d_hand_loss_panopticDB(pred_right_hand_joints_3d,
                                                                           pred_left_hand_joints_3d, gt_lhand_3d,
                                                                           gt_rhand_3d)
            ...
            for b in range(batch_size):
                ...
                if self.options.bUseSMPLX:
                    for pred_rl in [pred_right_hand_joints_3d, pred_left_hand_joints_3d]:
                        ...
                        pred_joints_vis = convert_smpl_to_bbox(pred_joints_vis, camParam_scale, camParam_trans)
                        pred_joints_vis = pred_joints_vis.ravel()[:, np.newaxis]
                        glViewer.addSkeleton([pred_joints_vis], colorRGB=(0, 0, 255),
                                             jointType='hand_panopticdb')  # GT: blue
    ...
    def run_smplify(self, input_batch):
        ...
        if self.options.bDebug_visEFT:
            smpl_utils.visSMPLoutput_bboxSpace(self.smpl,
                                               {"pred_rotmat": init_pred_rotmat, "pred_shape": init_pred_betas,
                                                "pred_camera": init_pred_camera}, image=images[0])
            ...
            curImgVis = viewer2D.Vis_Skeleton_2D_SPIN49(gt_keypoints_2d_orig_vis[0, :, :2],
                                                        gt_keypoints_2d_orig_vis[0, :, 2] > 0.1, bVis=False,
                                                        image=curImgVis)
        ...
        pred_rotmat_hom = torch.cat(
            [init_pred_rotmat.detach().view(-1, 3, 3).detach(), torch.tensor([0, 0, 1], dtype=torch.float32,
                                                                             device=self.device).view(1, 3, 1).expand(
        ...
        if self.options.bDebug_visEFT:
            print("Final")
            smpl_utils.visSMPLoutput_bboxSpace(self.smpl, {"pred_rotmat": new_opt_rot_mats, "pred_shape": new_opt_betas,
                                                           "pred_camera": new_opt_cam_t}, image=images[0])
    ...
    def eftAllInDB(self, eft_out_dir="./eft_out/", bExportPKL=True, test_dataset_3dpw=None, test_dataset_h36m=None):
        ...
        for step, batch in enumerate(tqdm(train_data_loader)):  # , desc='Epoch '+str(epoch),
            ...
            for it in range(maxExemplarIter):
                ...
                if True:
                    print(output['loss_keypoints_2d'])
    ...
    # Run SMPLify
    # Save output as seperate pkl files
    def smplifyAllInDB(self, smplify_out_dir="./smplify_out/", bExportPKL=True, test_dataset_3dpw=None,
                       test_dataset_h36m=None):
