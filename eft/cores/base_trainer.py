...
class BaseTrainer(object):
    ...
    # def train(self, test_dataset_3dpw, test_dataset_3dpw_crop, test_dataset_3dpw_headcrop, test_dataset_h36m):
    def train(self, test_dataset_list):
        '''
            test_dataset_list: list of [testdb_instance, 'dataset_name'] 
        '''
        """Training process."""
        ...
        if self.options.noEval ==False:       #Debug
            for test_db in test_dataset_list:
                dbname = test_db[1]
                best_error = self.test(test_db[0], dbname)
                self.summary_writer.add_scalar('{}_err_mm'.format(dbname), best_error, self.step_count)
                bestTestErr[dbname]  = best_error
            ...
            # cur_error_3dpw_crop = self.test(test_dataset_3dpw_crop, '3dpw-headcrop')
            # self.summary_writer.add_scalar('3dpw_crop_test_err_mm', cur_error_3dpw_crop, self.step_count)
        ...
        for epoch in tqdm(range(self.epoch_count, self.options.num_epochs), total=self.options.num_epochs, initial=self.epoch_count):
            ...
            for step, batch in enumerate(tqdm(train_data_loader, desc='Epoch '+str(epoch),
                ...
                #Save every (big) interval of iterations
            ...
            if (epoch+1) % self.options.test_epoch_inter == 0:
                ...
                for test_db in test_dataset_list:
                    dbname = test_db[1]
                    cur_error = self.test(test_db[0], dbname)
                    self.summary_writer.add_scalar('{}_test_err_mm'.format(dbname), cur_error, self.step_count)

                    if bestTestErr[dbname] > cur_error and dbname=='3dpw':
                    # if best_error_3dpw>cur_error_3dpw: #or >cur_error_h36m:
                        print(">>> Great! Found a new best model. Save this!")
                        self.saver.save_checkpoint(self.models_dict, self.optimizers_dict, epoch+1, 0, self.options.batch_size, None, self.step_count, suffix='best-{}-{}'.format(dbname, cur_error))
                        bestTestErr[dbname] = cur_error #New best
